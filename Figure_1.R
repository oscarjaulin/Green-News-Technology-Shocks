...
#jpeg(paste0('index_plot', ".jpeg"), width = 350, height = 350)
  ...
  plot(DATA$Date, exp(DATA$GPBII), type = "l", col = "chartreuse4", ylab = "", xlab = "Period", lwd = 2)
  axis(side = 2, col.axis = "chartreuse4")  # Left axis in green
  
  # Add NGPBII (right axis, red)
  par(new = TRUE)
  plot(DATA$Date, exp(DATA$NGPBII), type = "l", col = "brown", axes = FALSE, xlab = "", ylab = "", lwd = 2)
  axis(side = 4, col.axis = "brown")  # Right axis in red
  # Add legend
  legend("topleft", legend = c("GPBII", "NGPBII"), col = c("chartreuse4", "brown"), lwd = 2, bty = "n")

#dev.off()

# Plot 2: Ratio
# Calculate ratio
ratio <- exp(DATA$GPBII) / exp(DATA$NGPBII)
...
GPBII_growth <- c(NA, diff((DATA$GPBII)))
NGPBII_growth <- c(NA, diff((DATA$NGPBII)))
