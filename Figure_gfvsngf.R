...
GFPBII_growth <- c(NA, diff((DATA$GFGPBII)))
NGFPBII_growth <- c(NA, diff((DATA$NGFGPBII)))

jpeg(paste0('index_growth_plot', ".jpeg"), width = 350, height = 350)

# Plot
plot(DATA$Date, GFPBII_growth, type = "l", col = "chartreuse4", ylab = "percent", xlab = "Period", lwd = 2)
lines(DATA$Date, NGFPBII_growth, col = "brown", lwd = 2)
